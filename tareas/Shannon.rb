...
metodo = ""
while true
  puts "Ingrense metodo de entrada de datos [(N)umerico/(T)exto]"
  metodo = gets.chomp.downcase

  if metodo == "t" || metodo == "n"
    break
  else
    puts "Valoes no validos, ingrese 'N' para numerico o 'T' para texto" 
    puts ""
  end
end
...
while true
  if metodo == "n"
    ...
    if dics.keys.count <= 1
      puts "Datos deben de incluir mas de una letra en el diccionario"
      puts ""
    else
      dics.each do |key, value|
        datas.push("#{value}/#{array.count}".to_r)
      end
...
puts "R => #{ ( 8 / sumLs ).to_f.round(4) } bits/s"
